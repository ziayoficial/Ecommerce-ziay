'use client'
// SPRINT-ADMIN-INCIDENTS-001 — admin incident management UI.
//
// Admin-only page for managing status page incidents. Lists all incidents
// (active + recently resolved), lets the admin create new ones, advance
// their status (investigating → identified → monitoring → resolved), and
// append timeline messages that get persisted into the `updates` JSON
// column on `StatusIncident`.
//
// Auth model:
//   - The route itself is NOT in `PUBLIC_PATTERNS` in `src/middleware.ts`,
//     so the middleware already requires a valid NextAuth JWT to reach it
//     (a logged-out user is redirected to /login before this component
//     even mounts).
//   - The middleware does NOT distinguish admin vs. non-admin (it can't —
//     the JWT's `role` claim isn't an Edge-compatible lookup). So this
//     page additionally checks `session.user.role === 'admin'` client-side
//     and hard-redirects non-admins to `/`. The route handler
...
export default function AdminIncidentsPage() {
  ...
  // Non-admins get bounced to `/`. Wait for the session to load first so
  // we don't flash the redirect on a slow connection.
  useEffect(() => {
  ...
  // Session still loading → show skeletons so we don't flash the redirect
  // for a logged-in admin on a slow connection.
  ...
  // Non-admin → render nothing (the effect above is redirecting).
  if (session && session.user.role !== 'admin') {
    return null
  }
