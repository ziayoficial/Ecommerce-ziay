...
**Lo que yo construí** (ZIAY) es la **capa de operación + dashboard + atribución**: Next.js 16 + Prisma + Socket.io + LLM de smart reply + motor CPA/ROAS/ROI + mensajería omnicanal (WhatsApp + Messenger + IG) + estrategia de pago configurable.
...
| Tema | Documento Saramantha | ZIAY (construido) | Estado |
|---|---|---|---|
...
- **ZIAY:** WhatsApp (CO primario + MX), Messenger (internacional), Instagram DM. Bandeja unificada con badges por canal.
...
- **ZIAY:** tabla `Ad`, `AdSpend` (diario), `Attribution` (multi-touch ready), motor `/api/ads` que calcula CPA, ROAS, ROI, CPL, CVR, AOV por anuncio, y emite veredictos: **escalar / optimizar / vigilar / pausar / apagar / canibaliza**. Kill-switch con audit log.
...
- **Veredicto:** ✅ Quedarse. **Es la pieza más diferenciadora del proyecto**. El documento Saramantha trata la atribución como un dato; ZIAY la trata como un motor accionable.
...
- **ZIAY:** `Channel.paymentStrategy ∈ {advance, cod, hybrid}` + `requirePrepayMin` + `prepayDiscountPct` + `codFee`. UI de configuración por canal y país. Recomendaciones contextuales al agente en el panel del cliente.
...
- **ZIAY:** 1 solo endpoint `/api/ai-reply` con un system prompt genérico.
...
- **ZIAY:** solo `Product` interno, sin adaptadores.
...
- **ZIAY:** tengo VLM disponible pero no implementé este flujo.
...
- **ZIAY:** vista de tabla interna en `/orders`. No es Kanban.
...
- **ZIAY:** `Setting` key-value genérico + `Channel` con estrategia de pago. No hay "tono de marca" ni "nombre de asesora" ni "pregunta de perfil".
...
| Hallazgo (de 239 pedidos reales) | Implicación para ZIAY |
|---|---|
...
- **ZIAY:** Next.js API Routes + LLM SDK directo.
...
- **ZIAY:** SQLite + Prisma (portable a Postgres).
...
**No tires nada de lo construido.** ZIAY te da tres cosas que tu documento Saramantha no tenía (multi-canal, motor de atribución, estrategia de pago configurable) y que vos me pediste explícitamente. Lo que hay que hacer es **evolucionar el núcleo** para incorporar las 8 brechas críticas del documento Saramantha, en el orden del plan de Sprint 1-5.

El resultado final sería una plataforma que combina:

- **Capa de operación (ZIAY)** — dashboard omnicanal, motor de atribución, estrategia de pago, kill-switch de pauta.
...
*Documento de revisión generado tras lectura al 100% de `PROYECTO_saramantha_agentes_whatsapp.md` (922 líneas) y comparación contra el estado actual de ZIAY.*
