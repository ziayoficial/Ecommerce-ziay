...
describe.skipIf(!process.env.LLM_API_KEY)('Agent evaluation — novedades (LLM-as-judge)', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    chatMock.mockResolvedValue({
      content: JSON.stringify({
        ...
        reasoning: 'Novedad message asks for the corrected address without blaming the customer.',
      }),
      model: 'glm-4.6-plus',
      provider: 'zai',
      usage: { promptTokens: 480, completionTokens: 75, totalTokens: 555 },
    })
  })
  ...
  it('parses the judge JSON response and returns per-criterion scores', async () => {
    ...
    const judgeResult = await evaluateWithLLMJudge(
      CASES[0],
      'Novedad con tu envío: la dirección no está completa. ¿Me confirmas tu dirección actual en Bogotá?',
      {
    ...
    // Equal-weight average: (0.95 + 0.9 + 0.85 + 0.9 + 0.95) / 5 = 0.91
    expect(judgeResult.score).toBeCloseTo(0.91, 1)
